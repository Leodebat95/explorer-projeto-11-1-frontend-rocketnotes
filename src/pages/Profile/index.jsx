...
export function Profile() {
  ...
  const [avatarFile, setAvatarFile] = useState(null)
    /* state que guarda o arq. (a nova imagem) selecionada-uploadada pelo usuário;
       null --- state inicial é sem o arq da nova img */
  const avatarUrl = user.avatar ? `${api.defaults.baseURL}/files/${user.avatar}` : avatarPlaceholder
    /* se o usuário tem um avatar:
            ? vai ser posto o arq-já existente da url (do back-end);
            : se não, é posto o "avatarPlaceholder" */
  ...
  async function handleUpdate() {
    ...
    const userUpdated = Object.assign(user, updated)
      /* Object.assign(target, source) --- Method pra copiar os valores de um obj. de origem, 
                                           pra um obj. de destino;
